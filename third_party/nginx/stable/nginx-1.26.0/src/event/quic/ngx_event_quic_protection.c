...
/* Include the new types header file */
...
struct ngx_quic_connection_s {
    ...
    unsigned                          closing:1;
    unsigned                          draining:1;
    unsigned                          key_update_needed:1;
    unsigned                          key_update_conf:1;
};

void ngx_quic_run(ngx_connection_t *c, ngx_quic_conf_t *conf);
void ngx_quic_shutdown(ngx_connection_t *c, ngx_uint_t err, const char *reason);
void ngx_quic_close_connection(ngx_connection_t *c, ngx_uint_t err,
    const char *reason);
void ngx_quic_finalize_connection(ngx_connection_t *c, ngx_uint_t err,
    const char *reason);
ngx_int_t ngx_quic_reset_stream(ngx_connection_t *c, ngx_uint_t sid);
ngx_int_t ngx_quic_shutdown_stream(ngx_connection_t *c, ngx_uint_t sid);
void ngx_quic_discard_ctx(ngx_connection_t *c, enum ssl_encryption_level_t level);
ngx_quic_frame_t *ngx_quic_alloc_frame(ngx_connection_t *c);
void ngx_quic_free_frame(ngx_connection_t *c, ngx_quic_frame_t *f);
ngx_quic_path_t *ngx_quic_find_path(ngx_connection_t *c,
    struct sockaddr *sockaddr);
void ngx_quic_set_connection_path(ngx_connection_t *c, ngx_quic_path_t *path);
void ngx_quic_congestion_lost(ngx_connection_t *c, ngx_quic_frame_t *f);
void ngx_quic_rbtree_insert_value(ngx_rbtree_node_t *temp,
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel);
ngx_int_t ngx_quic_update_keys(ngx_connection_t *c);
void ngx_quic_set_lost(ngx_connection_t *c, ngx_quic_frame_t *f);
void ngx_quic_empty_handler(ngx_event_t *ev);
void ngx_quic_default_log_handler(ngx_log_t *log, ngx_uint_t level,
    const char *buf, ngx_uint_t len);

extern ngx_uint_t  ngx_quic_max_udp_payload_size;
