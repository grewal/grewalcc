...
func main() {
	...
	httpListenAddr := os.Getenv("HTTP_LISTEN_ADDR"); if httpListenAddr == "" { httpListenAddr = ":9001" }
	grpcListenAddr := os.Getenv("GRPC_LISTEN_ADDR"); if grpcListenAddr == "" { grpcListenAddr = ":9002" }
	...
	consulClient, err := consulapi.NewClient(consulConfig); if err != nil {
		logger.Error("Fatal error creating Consul client", "error", err, "address", consulConfig.Address); os.Exit(1)
	}
	...
	pingErr := rdb.Ping(ctxPing).Err(); cancelPing()
	if pingErr != nil {
		logger.Error("Fatal error connecting to Redis", "address", redisAddr, "error", pingErr); os.Exit(1)
	}
	...
	go func() {
		...
		} else { httpServerErrChan <- nil }
	}()

	grpcListener, err := net.Listen("tcp", grpcListenAddr); if err != nil {
		logger.Error("Failed to listen for gRPC", "address", grpcListenAddr, "error", err); os.Exit(1)
	}
	...
	case err := <-httpServerErrChan:
		if err != nil { logger.Error("HTTP Server failed or stopped unexpectedly", "error", err); grpcServer.Stop()
		} else { logger.Info("HTTP Server stopped.") }
	case err := <-grpcServerErrChan:
		if err != nil { logger.Error("gRPC Server failed or stopped unexpectedly", "error", err); _ = httpServer.Shutdown(context.Background())
	...
	close(quit)
	waitTimeout := 20 * time.Second; waitChan := make(chan struct{})
	go func() { wg.Wait(); close(waitChan) }()
	select {
	case <-waitChan: logger.Info("All background tasks completed.")
	case <-time.After(waitTimeout): logger.Warn("Timeout waiting for background tasks to complete.")
	}

	if err := rdb.Close(); err != nil { logger.Error("Error closing Redis client", "error", err)
	} else { logger.Debug("Redis client closed.") }
	
	grpcServer.GracefulStop(); logger.Info("gRPC server shutdown gracefully.")
	...
	if err := httpServer.Shutdown(shutdownCtx); err != nil { logger.Error("Graceful HTTP server shutdown failed", "error", err)
	} else { logger.Info("HTTP server shutdown gracefully.") }
