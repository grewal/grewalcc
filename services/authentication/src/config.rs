...
pub async fn load_config() -> anyhow::Result<AppConfig> {
    ...
    let db_c = DbConfig {
        ...
        acquire_timeout_seconds: env::var("DB_ACQUIRE_TIMEOUT_SECONDS").ok().and_then(|s| s.parse().ok()).unwrap_or(DbConfig::default().acquire_timeout_seconds), // Added
        idle_timeout_seconds: env::var("DB_IDLE_TIMEOUT_SECONDS").ok().and_then(|s| s.parse().ok()).unwrap_or(DbConfig::default().idle_timeout_seconds),     // Added
        max_lifetime_seconds: env::var("DB_MAX_LIFETIME_SECONDS").ok().and_then(|s| s.parse().ok()).unwrap_or(DbConfig::default().max_lifetime_seconds),   // Added
    };
